...
// GameToken with Governance.
contract PolyGamerToken is BEP20('PolyGamer Token', 'GAME') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "GAME::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "GAME::delegateBySig: invalid nonce");
        require(now <= expiry, "GAME::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "GAME::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GAMEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "GAME::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of game. He can make Game and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that GAMEs distribution occurs.
        uint256 accGamePerShare;   // Accumulated GAMEs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The GAME TOKEN!
    PolyGamerToken public game;
    // Dev address.
    address public devaddr;
    // GAME tokens created per block.
    uint256 public gamePerBlock;
    ...
    constructor(
        PolyGamerToken _game,
        address _devaddr,
        address _feeAddress,
        uint256 _gamePerBlock,
        uint256 _startBlock
    ) public {
        game = _game;
        devaddr = _devaddr;
    ...
    function pendingGame(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gameReward = multiplier.mul(gamePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gameReward = multiplier.mul(gamePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        game.mint(devaddr, gameReward.div(10));
        game.mint(address(this), gameReward);
        pool.accGamePerShare = pool.accGamePerShare.add(gameReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount,address referrer) public   {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accGamePerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updateEmissionRate(uint256 _gamePerBlock) public onlyOwner {
        massUpdatePools();
        gamePerBlock = _gamePerBlock;
    }
